...
export function MySchedule({ ctx }: { ctx: AppCtx }) {
  ...
  const total = mine.length + myActs.length
  const exportIcs = () => {
    // activities map onto the same shape the ICS builder needs
    ...
    const ics = buildScheduleIcs([...mine, ...actEvents], ctx.days, ctx.event.startISO, ctx.event.location)
    ...
    <div>
      ...
      <div style={{ padding: '8px 12px ' + (TABBAR_H + 16) + 'px' }}>
        ...
        {ctx.days.map((d) => {
          ...
          if (day.length + acts.length === 0) return null
          // merge chronologically
          const items: ({ kind: 'session'; start: string; s: Session } | { kind: 'activity'; start: string; a: (typeof acts)[number] })[] = [
          ...
          return (
            <div key={d.id} style={{ marginBottom: 18 }}>
              ...
              <div style={{ background: 'var(--wf-surface)', borderRadius: 'var(--radius-5)', boxShadow: 'var(--shadow-card)', overflow: 'hidden' }}>
                {items.map((it, i) => (
                  <div key={it.kind === 'session' ? it.s.id : it.a.id} style={{ borderBottom: i === items.length - 1 ? 'none' : '1px solid ' + T.line }}>
                    {it.kind === 'session' ? (
