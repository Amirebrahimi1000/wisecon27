          ...
          <div key={q.id}>
            ...
            {q.kind === 'nps' && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {Array.from({ length: 11 }, (_, n) => (
                  <Press key={n} onClick={() => set(q.id, n)} style={{ width: 40, height: 40, borderRadius: 'var(--radius-3)', display: 'grid', placeItems: 'center', fontFamily: T.onest, fontWeight: 700, fontSize: 14, background: answers[q.id] === n ? T.green9 : '#fff', color: answers[q.id] === n ? '#fff' : T.body, boxShadow: answers[q.id] === n ? 'none' : 'inset 0 0 0 1px var(--wf-grey-6)' }}>{n}</Press>
                ))}
              </div>
            )}
            ...
            {q.kind === 'choice' && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {q.options.map((o) => (
                  <Press key={o} onClick={() => set(q.id, o)} style={{ fontFamily: T.sig, fontWeight: 600, fontSize: 13.5, padding: '8px 14px', borderRadius: 999, background: answers[q.id] === o ? T.green9 : '#fff', color: answers[q.id] === o ? '#fff' : T.body, boxShadow: answers[q.id] === o ? 'none' : 'inset 0 0 0 1px var(--wf-grey-6)' }}>{o}</Press>
                ))}
              </div>
            )}
