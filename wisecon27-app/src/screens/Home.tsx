...
function planForHome(ctx: AppCtx, clock: EventClock, t: (k: string) => string) {
  const dayId =
    clock.phase === 'before'
      ? ctx.days[0]?.id
  ...
  const mtgs: PlanItem[] = ctx.meetings
    ...
    .map((m) => ({
      ...
      title: t('home.meetingWith') + ' ' + ctx.nameFor(m.requesterId === ctx.userId ? m.inviteeId : m.requesterId),
      place: ctx.meetingPoints.find((p) => p.id === m.pointId)?.label ?? t('home.meetingPointTbc'),
      kind: 'meeting',
    }))
...
const QUICK: QuickAction[] = [
  { icon: 'message', labelKey: 'home.quickCommunity', push: 'community' },
  { icon: 'map', labelKey: 'home.quickVenueMap', push: 'venuemap' },
  { icon: 'speakers', labelKey: 'home.quickSpeakers', tab: 'speakers' },
  { icon: 'info', labelKey: 'home.quickInfo', push: 'info' },
]
    ...
    return (
      <div style={{ padding: '24px 16px 0' }}>
        <Press onClick={() => ctx.push('editprofile', { focus: 'interests' })} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--wf-surface)', borderRadius: 'var(--radius-4)', padding: 16, boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 14.5, color: T.ink }}>{t('home.getPersonalSuggestions')}</div>
            <div style={{ fontFamily: T.sig, fontSize: 13, color: T.muted, marginTop: 1, lineHeight: 1.4 }}>{t('home.getPersonalSuggestionsBody')}</div>
          </div>
  ...
  return (
    <div style={{ paddingTop: 24 }}>
      <div style={{ padding: '0 16px' }}>
        <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 19, color: T.ink, marginBottom: 4 }}>{t('home.suggestedForYou')}</div>
        <div style={{ fontFamily: T.sig, fontSize: 13, color: T.muted, marginBottom: 12, lineHeight: 1.4 }}>
          {t('home.suggestedForYouSubtitle')}
        </div>
      </div>
      {sessions.length > 0 && (
      ...
      {people.length > 0 && (
        <div style={{ padding: '16px 16px 0' }}>
          <Eyebrow style={{ marginBottom: 10, paddingLeft: 2 }} color={T.subtle}>{t('home.peopleYouShouldMeet')}</Eyebrow>
            ...
            {people.map((p) => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--wf-surface)', borderRadius: 'var(--radius-4)', padding: 12, boxShadow: 'var(--shadow-sm)' }}>
                ...
                <Press onClick={() => ctx.push('delegate', { peerId: p.id })} style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
                  ...
                  <span style={{ display: 'inline-block', marginTop: 4, fontFamily: T.onest, fontSize: 11, color: T.green10, background: T.green1, borderRadius: 999, padding: '2px 8px' }}>{p.mutual} {t('home.sharedInterests')}</span>
                </Press>
                <Btn kind="primary" size="sm" onClick={() => { ctx.setConnection(p.id, 'pending'); ctx.toast(t('home.requestSentTo') + ' ' + p.name.split(' ')[0]) }}>{t('home.connect')}</Btn>
              </div>
            ))}
          </div>
        </div>
...
function PostEventSection({ ctx }: { ctx: AppCtx }) {
  ...
  const exportConnections = async () => {
    if (exporting) return
    if (!connected.length) return ctx.toast(t('home.noConnectionsToExport'))
    setExporting(true)
    ...
    ctx.toast(t('home.connectionsExported'))
  }

  return (
    <div style={{ padding: '20px 16px 0' }}>
      <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 19, color: T.ink, marginBottom: 4 }}>{t('home.afterTheEvent')}</div>
      <div style={{ fontFamily: T.sig, fontSize: 13, color: T.muted, marginBottom: 12, lineHeight: 1.4 }}>
        {t('home.afterTheEventSubtitle')}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <Press onClick={exportConnections} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--wf-surface)', borderRadius: 'var(--radius-4)', padding: 16, boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 14.5, color: T.ink }}>{exporting ? t('home.exporting') : t('home.exportMyConnections')}</div>
            <div style={{ fontFamily: T.sig, fontSize: 12.5, color: T.muted, marginTop: 1 }}>{connected.length} {connected.length === 1 ? t('home.contactCsvSingular') : t('home.contactCsvPlural')}</div>
          </div>
          <Icon name="chevronRight" size={18} stroke={2} style={{ color: T.muted }} />
        </Press>
        {slides.length > 0 && (
          <div style={{ background: 'var(--wf-surface)', borderRadius: 'var(--radius-4)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
            <Eyebrow style={{ padding: '14px 16px 4px' }}>{t('home.sessionSlides')}</Eyebrow>
            {slides.map((s, i) => (
              <a
                key={s.id}
      ...
      <Press onClick={() => ctx.push('meetings', {})} style={{ display: 'flex', alignItems: 'center', gap: 12, background: T.green9, borderRadius: 'var(--radius-5)', padding: 16, color: '#fff', boxShadow: 'var(--shadow-card)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 14.5 }}>
            {pending.length} {pending.length === 1 ? t('home.meetingRequestWaiting') : t('home.meetingRequestsWaiting')}
          </div>
        ...
        <span style={{ fontFamily: T.sig, fontWeight: 600, fontSize: 13, flexShrink: 0 }}>{t('home.respond')}</span>
...
function HomeBold({ ctx }: { ctx: AppCtx }) {
  ...
  const { upNext, later, mine } = planForHome(ctx, clock, t)
  const track = upNext && upNext.kind === 'session' && upNext.session ? trackOf(upNext.session.track) : null
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 8 }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontFamily: T.onest, fontWeight: 600, fontSize: 11.5, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.85)', whiteSpace: 'nowrap' }}>
              ...
              {live ? `Live · Day ${clock.dayIndex} of ${clock.total}`
                : clock.phase === 'before' ? t('home.countingDown')
                : clock.phase === 'ended' ? t('home.eventEnded')
                : ctx.event.dateline}
            </span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <Press onClick={() => ctx.push('editprofile', {})} aria-label={t('home.yourProfile')} style={{ flexShrink: 0 }}>
          ...
          <div style={{ marginTop: clock.phase === 'before' ? 26 : 22 }}>
            {clock.phase === 'before' ? (
              ...
              <>
                <div style={{ fontFamily: T.onest, fontSize: 14, color: 'rgba(255,255,255,0.85)' }}>{t('home.thanksForJoining')}, {me.name.split(' ')[0]}</div>
                <h1 style={{ fontFamily: T.onest, fontWeight: 700, fontSize: 32, color: '#fff', lineHeight: 1.05, letterSpacing: '-0.02em', marginTop: 6 }}>{t('home.seeYouAtWisecon28')}</h1>
              </>
            ) : (
              <div style={{ fontFamily: T.onest, fontWeight: 700, fontSize: 26, color: '#fff', letterSpacing: '-0.01em' }}>{t('home.greetingMorning')}, {me.name.split(' ')[0]}</div>
            )}
          </div>
          ...
          {clock.phase === 'before' && clock.countdown ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginTop: 20, background: 'rgba(255,255,255,0.15)', borderRadius: 'var(--radius-4)', padding: '14px 16px', backdropFilter: 'blur(4px)' }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: T.sig, fontSize: 12.5, color: 'rgba(255,255,255,0.82)', marginTop: 3 }}>{t('home.untilWisecon27Begins')}</div>
              </div>
            </div>
          ) : clock.phase === 'ended' ? (
            ctx.surveyDone ? (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 18, fontFamily: T.sig, fontSize: 13.5, color: 'rgba(255,255,255,0.92)' }}>
                <Icon name="checkCircle" size={16} /> {t('home.thanksForFeedback')}
              </div>
            ) : (
              <Press onClick={() => ctx.push('survey', {})} style={{ display: 'flex', alignItems: 'center', gap: 13, marginTop: 20, background: '#fff', borderRadius: 'var(--radius-4)', padding: '14px 16px', boxShadow: 'var(--shadow-card)' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 15, color: T.ink }}>{t('home.howWasWisecon27')}</div>
                  <div style={{ fontFamily: T.sig, fontSize: 12.5, color: T.muted, marginTop: 1 }}>{t('home.takePostConferenceSurvey')}</div>
                </div>
                <Icon name="chevronRight" size={18} stroke={2} style={{ color: T.muted }} />
              </Press>
            )
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <HeroStat n={ctx.sessions.filter((s) => s.day === ctx.days[clock.dayIndex - 1]?.id && s.type !== 'break').length} label={t('home.sessionsToday')} />
              <HeroStat n={mine.length} label={t('home.inYourPlan')} />
              <HeroStat n={ctx.speakers.length} label={t('home.speakers')} />
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ marginTop: -34, background: 'var(--wf-grey-2)', borderRadius: '24px 24px 0 0', position: 'relative', paddingTop: 22 }}>
        ...
        {upNext && clock.phase !== 'ended' && (
          <div style={{ padding: '0 16px' }}>
            ...
            <Press onClick={() => (upNext.kind === 'session' ? ctx.openSession(upNext.session!) : ctx.push(upNext.kind === 'meeting' ? 'meetings' : 'activities', {}))} style={{ background: 'var(--wf-surface)', borderRadius: 'var(--radius-5)', overflow: 'hidden', boxShadow: 'var(--shadow-card)', display: 'flex' }}>
              <div style={{ width: 6, background: track ? track.dot : T.green9, flexShrink: 0 }} />
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  {upNext.kind === 'session' ? (
                    ...
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: T.onest, fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.04em', color: T.green10, background: T.green1, borderRadius: 999, padding: '4px 11px' }}>
                      <Icon name={upNext.kind === 'meeting' ? 'connect' : 'sparkles'} size={13} /> {upNext.kind === 'meeting' ? t('home.oneOnOneMeeting') : t('home.activity')}
                    </span>
                  )}
                  {upNext.kind === 'session' && (
        ...
        <div style={{ position: 'relative' }}>
          <div ref={quickRef} onScroll={checkQuick} className="wc-noscroll" style={{ display: 'flex', gap: 10, padding: '18px 16px 0', overflowX: 'auto' }}>
            {QUICK.map((q, i) => (
              <Press key={q.labelKey} onClick={() => runQuick(ctx, q)} style={{ position: 'relative', flex: '1 0 92px', display: 'flex', flexDirection: 'column', gap: 12, padding: 14, borderRadius: 'var(--radius-4)', background: i === 0 ? T.green9 : 'var(--wf-surface)', color: i === 0 ? '#fff' : T.ink, boxShadow: i === 0 ? 'none' : 'var(--shadow-sm)' }}>
                <Icon name={q.icon} size={22} style={{ color: i === 0 ? '#fff' : T.green10 }} />
                <span style={{ fontFamily: T.sig, fontWeight: 600, fontSize: 12.5 }}>{t(q.labelKey)}</span>
        ...
        <div style={{ padding: '24px 16px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
            <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 19, color: T.ink }}>{t('home.yourPlan')}</div>
            <Press onClick={() => ctx.setTab('agenda')} style={{ fontFamily: T.sig, fontWeight: 600, fontSize: 13.5, color: T.green10 }}>{t('home.fullProgramme')}</Press>
          </div>
          <div style={{ fontFamily: T.sig, fontSize: 13, color: T.muted, marginBottom: 12, lineHeight: 1.4 }}>
            {live ? t('home.yourPlanSubtitleLive') : t('home.yourPlanSubtitle')}
          </div>
          {mine.length === 0 ? (
            <Press onClick={() => ctx.setTab('agenda')} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--wf-surface)', borderRadius: 'var(--radius-4)', padding: 16, boxShadow: 'var(--shadow-sm)' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: T.sig, fontWeight: 700, fontSize: 14.5, color: T.ink }}>{t('home.noSavedSessions')}</div>
                <div style={{ fontFamily: T.sig, fontSize: 13, color: T.muted, marginTop: 1 }}>{t('home.noSavedSessionsBody')}</div>
              </div>
              <Icon name="chevronRight" size={18} stroke={2} style={{ color: T.muted }} />
            </Press>
          ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {later.length === 0 && (
              <div style={{ fontFamily: T.sig, fontSize: 13.5, color: T.muted, padding: '4px 2px', lineHeight: 1.45 }}>{t('home.onlySavedSession')}</div>
            )}
          </div>
          )}
        </div>
