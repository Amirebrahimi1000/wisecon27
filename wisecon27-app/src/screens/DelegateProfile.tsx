...
export function DelegateProfile({ ctx }: { ctx: AppCtx }) {
  ...
  return (
    <div style={{ minHeight: '100%', background: 'var(--wf-grey-2)' }}>
      <AppHeader title={t('delegate.title')} onBack={ctx.back} />
      <div style={{ padding: '14px 16px ' + (TABBAR_H + 16) + 'px' }}>
        {missing && <Eyebrow>{t('delegate.notAvailable')}</Eyebrow>}
        {p && (
          <>
            {/* identity card */}
            <div style={{ background: 'var(--wf-surface)', borderRadius: 'var(--radius-5)', boxShadow: 'var(--shadow-card)', padding: 20, textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 16, flexWrap: 'wrap' }}>
                {incoming ? (
                  <>
                    <Btn kind="default" size="sm" onClick={() => { ctx.declineConnection(p.id); ctx.back() }}>{t('delegate.ignore')}</Btn>
                    <Btn kind="primary" size="sm" onClick={() => ctx.acceptConnection(p.id)}>{t('delegate.acceptRequest')}</Btn>
                  </>
                ) : att?.status === 'connected' ? (
                  <Btn kind="primary" size="sm" icon="message" onClick={() => ctx.push('conversation', { peerId: p.id })}>{t('delegate.message')}</Btn>
                ) : att?.status === 'pending' ? (
                  <Btn kind="default" size="sm" onClick={() => ctx.setConnection(p.id, 'connect')}>{t('delegate.requestedCancel')}</Btn>
                ) : (
                  <Btn kind="primary" size="sm" icon="connect" onClick={() => { ctx.setConnection(p.id, 'pending'); ctx.toast(t('delegate.requestSent')) }}>{t('delegate.connect')}</Btn>
                )}
                {!incoming && (
                  <Btn kind="outline" size="sm" icon="calendar" onClick={() => ctx.push('meetingrequest', { peerId: p.id })}>{t('delegate.suggestMeeting')}</Btn>
                )}
              </div>
            </div>
            ...
            {shared('bio') && p.bio.trim() && (
              <div style={{ background: 'var(--wf-surface)', borderRadius: 'var(--radius-5)', boxShadow: 'var(--shadow-card)', padding: 16, marginTop: 14 }}>
                <Eyebrow style={{ marginBottom: 8 }}>{t('delegate.about')}</Eyebrow>
            ...
            {shared('interests') && p.interests.length > 0 && (
              <div style={{ background: 'var(--wf-surface)', borderRadius: 'var(--radius-5)', boxShadow: 'var(--shadow-card)', padding: 16, marginTop: 14 }}>
                <Eyebrow style={{ marginBottom: 10 }}>{t('delegate.interests')}</Eyebrow>
            ...
            {shared('linkedin') && p.linkedin.trim() && (
              ...
              >
                ...
                <span style={{ flex: 1, fontFamily: T.sig, fontWeight: 600, fontSize: 14.5, color: T.ink }}>{t('delegate.linkedin')}</span>
